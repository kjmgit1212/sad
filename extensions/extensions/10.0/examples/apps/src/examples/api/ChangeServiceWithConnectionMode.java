...
/**
 * Example purpose: Change the connection mode of a service.
 */
public class ChangeServiceWithConnectionMode {
	...
	public static boolean run(String[] args, boolean verbose) {
		...
		try {
			...
			String connectionMode = (String)arguments.get(CONNECTION_MODE);
			utils.print("------ serviceName:"+ serviceName);
			utils.print("------ connectionMode:"+ connectionMode);
			...
			if (serviceIte.hasNext()) {
				...
				utils.print("\n\n****** Update the connection mode of the service and assign a default work order...");
	            ...
	            Service service = serviceMO.getData();
	            service.setConnectionMode(cm);
	            
	            serviceMO.update(service, manualWorkOrder);
				
				utils.print("\n\nService connection mode updated successfully: "+ serviceMO.getDistinguishedName());
			}
	...
	public static String getUsage(String msg) {
		...
		usage.append("\nchangeServiceWithConnectionMode: " + msg + "\n");
		usage.append("usage: changeServiceWithConnectionMode -[argument] ? \"[value]\"\n");
		usage.append("\n");
		usage.append("-service\tName of the service to be updated\n");
        usage.append("-connectionmode\tConnection mode of the new service\n");
		usage.append("\n");
		usage.append("Example: changeServiceWithConnectionMode -service?\"WinLocal\" -connectionmode?\"MANUAL\"");
