...
import examples.api.recon.callback.ReconCompleteCallback;
...
/**
 * Sample command-line Java class to run recon. This example adds
 * a new recon unit and runs reconciliation with this unit. 
 *
 * To run a recon using existing reconciliation unit -
 * 1. Modify program to accept only schedule information of an existing
 *    reconciliation unit.
 * 2. Use 'ReconManager.getReconUnits(serviceMO);' to get all reconciliation
 *    units, and identify the one having the same schedule as specified
 *    as input.
 * 3. Use this reconciliation unit or ReconUnitData object as a parameter
 *    to one of the 'runRecon' functions of class - 
...
public class RunRecon { 
   ...
   private static final String RECON_FILTER       = "reconfilter";
   private static final String EXCLUDE_ATTRIBUTES = "excludeattributes";
   private static final String LOCK_SERVICE       = "lockservice";
   private static final String MAX_DURATION       = "maxduration";
   private static final String RECON_CALLBACK     = "reconcallback";
   ...
   public static void main(String[] args) {
        ...
        try {
           ...
           newReconUnitData.setMonth(month);
           ...
           if((String)arguments.get(DAY) != null) {
               ...
               newReconUnitData.setDayOfMonth(day);
           }

            //Set day of week
            //Should be set only if the day of month is not required
            if((String)arguments.get(DAY) == null && (String)arguments.get(DAY_OF_WEEK) != null) {
                int dayOfWeek = Integer.parseInt((String)arguments.get(DAY_OF_WEEK));
                if(dayOfWeek < 0 || dayOfWeek > 7) {
                    System.out.println("Invalid day of week specified. " + 
                        "Valid values 1-7 (1 = Sunday, 2 = Monday, ...) and 0 to ignore.\n");
                    System.exit(0);
                }
                newReconUnitData.setDayOfWeek(dayOfWeek);
            }
           ...
           // Set reconciliation filter if supported
           String reconQueryFilter  = (String) arguments.get(RECON_FILTER);
           if(reconManager.isFilterSupported(serviceMO)) {
               newReconUnitData.setReconQueryFilter(reconQueryFilter);
           } else {
               System.out.println("Recon query filter not supported, query is ignored.\n");
           }
           ...
           String excludeAttributes = (String) arguments.get(EXCLUDE_ATTRIBUTES);
           if(excludeAttributes != null) {

               String[] excludeAttribList = excludeAttributes.split(",");
               ArrayList<String> excludeQueryAttribures = new ArrayList<String>();
               for(int i=0;i<excludeAttribList.length; i++) {
                  excludeQueryAttribures.add(excludeAttribList[i]);
               }
               newReconUnitData.setReconQueryAttributes(attributes, 
                                         excludeQueryAttribures);
           } else {
                ArrayList<String> tobeReconciledAttributes = new ArrayList<String>(0);
                tobeReconciledAttributes.addAll(attributes);
                newReconUnitData.setReconQueryAttributes(tobeReconciledAttributes);
           }


           System.out.println("Checking if an existing reconciliation unit " +
                              " exists with the same schedule information.\n");
           Collection reconUnits = reconManager.getReconUnits(serviceMO);
           if(!reconUnits.isEmpty()) {
            Iterator it = reconUnits.iterator();
            while(it.hasNext()) {
                ReconUnitData reconUnitData = (ReconUnitData)it.next();
                if (reconUnitData.toString().equals(
                                           newReconUnitData.toString())) {
                    System.out.println("Reconciliation unit with the same " +
                    "schedule already exists, replacing the same with new " + 
                                                                "settings..");
                    it.remove();
                }
            }
           }
           reconUnits.add(newReconUnitData);

           System.out.println("Adding the new recon unit..\n");
           reconManager.setReconUnits(serviceMO, reconUnits);
           System.out.println("Submitting request to run recon");
           Collection existingReconUnits = reconManager.getReconUnits(serviceMO);
           for (Iterator itr = existingReconUnits.iterator(); itr.hasNext();)
           {
                ReconUnitData reconUnitData = (ReconUnitData)itr.next();
                if (reconUnitData.toString().
                        equals(newReconUnitData.toString())) {
                    newReconUnitData = reconUnitData;
                }
            }
           String reconcallback = (String) arguments.get (RECON_CALLBACK);
           Request request;
           if(Boolean.valueOf(reconcallback).booleanValue()) {
               ReconCompleteCallback reconCompleteCallback = 
                                            new ReconCompleteCallback();
               request = reconManager.runRecon(serviceMO, 
                                newReconUnitData, reconCompleteCallback);
           } else {
               request = reconManager.runRecon(serviceMO, newReconUnitData);
           }
   ...
   public static boolean checkArguments(Hashtable arguments) {
       ...
       if (!arguments.containsKey(NAME))
           return printUsage("Name not specified");
   ...
   public static boolean printUsage(String msg) {
       ...
       usage.append("\nrunRecon: " + msg + "\n");
       usage.append("usage: runRecon -[argument] ? \"[value]\"\n");
       usage.append("\n");
       ...
       usage.append("-name                name of recon.\n");
       ...
       usage.append("-[reconfilter]        Ldap filter to reconcile accounts. Not honored for supporting data only recon.\n");
       usage.append("-[excludeattributes] Attribute(s) separated by comma");
       usage.append(" to be excluded during \n                     reconciliation.\nNot honored for supporting data only recon.\n");
       ...
       usage.append("                     default value 600 minutes.\n");
       usage.append("-[reconcallback]     If the recon completion details needs to be logged in \n");
       usage.append("                     SystemOut.log. Default value \"false\"\n\n");
       usage.append("Arguments enclosed in [] are optional\n\n");
       usage.append("Example: runRecon -serviceprofile?WinLocalProfile -servicefilter?");
       usage.append("\"(erservicename=WinLocal)\" -reconfilter?\"(eruid=a*)\"");
       usage.append(" -name?\"ADServiceRecon\" -description?\"ADService recon unit\"");
       usage.append(" -querymode?0 -excludeattributes?\"erntpasswordneverexpires,description\"");
       usage.append(" -maxduration?10 -month?10 -day?26 -hour?10 -minute?10 -reconcallback?\"true\"");
       ...
       usage.append("RunRecon.parseArgs() to identify the arguments prefixed with '+'");
